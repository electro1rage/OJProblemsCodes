...
public:
	double minDev(vector <int> a, int k) {
		int n = a.size(), i, j, l, m;

		sort(a.begin(), a.end());

		double MVAL = 2e9;
		double dp[55][55][55];
		REP(i, n + 1) REP(j, n + 1) REP(l, k + 1) dp[i][j][l] = MVAL;

		dp[0][0][0] = 0;
		REP(i, n) for (j = i; j < n; ++j) REP(l, k) {
			...
			double var = 0;
			for (m = i; m <= j; ++m) {
				var += (abs(a[m] - avg) * abs(a[m] - avg));
			}

			var = var / (j - i + 1);
			dp[j + 1][j + 1][l + 1] = min(dp[j + 1][j + 1][l + 1], dp[i][j][l] + var);
		}
