...
double getAverage(vector <string> ratings) {
	...
	for (int i = 0; i < sz; ++i) {
		if (find(v[i].begin(), v[i].end(), Elly) != v[i].end()) {
			v.erase(v.begin() + i);
			break;
		}
	}
	...
	double sum = 0, temp = 0;
	for (int i = 0; i < sz; ++i) {
		int accum = accumulate(v[i].begin(), v[i].end(), 0);
		int l = v[i].size();
	...
	if (!special)
		return (Elly + sum) / (sz + 1);

	double p = (n % R) / (double)R, q = 1 - p;

	return q * temp + p * (Elly + sum) / (sz + 1);
}
