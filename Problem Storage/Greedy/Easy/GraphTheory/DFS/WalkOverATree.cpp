...
const int MVAL = 1010101;

vector<int> adj[55];
int memo[55][55][101][2];
int dpth[55];
...
public:
	...
	int maxNodesVisited(vector <int> parent, int L) {
		...
		for (i = 0; i < n; ++i) adj[parent[i]].push_back(i + 1);

		dfs(0, 0);
		int maxi = -MVAL;
		for (i = 0; i < n + 1; ++i) maxi = max(maxi, dpth[i]);
		if (maxi >= L) return L + 1;

		int rem = L - maxi;
		rem /= 2;

		return min(n - maxi, rem) + maxi + 1;
	}
